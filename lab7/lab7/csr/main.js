...
const saltsUrl=`http://localhost/lab7/lab7/api/salts.php`;
const saltsTableBody=document.querySelector('#saltTable tbody');
const saltsForm=document.getElementById('saltForm');
const loginForm=document.getElementById('loginForm');
const typeDropdown=document.querySelector('#saltForm select[name="type_id"]')
const purposeDropdown=document.querySelector('#saltForm select[name="purpose_id"]')
const profileUrl=`http://localhost/lab7/lab7/api/profile.php`;
...
function displayTypes(){
    ...
    .then(data => {
        ...
        for (let i=0;i<types.length;i++){
            dropDownOptions+=`<option value="${types[i].id}">${types[i].name}</option>`;
            content+=`<tr>
                    <td>${types[i].id}</td>
                    <td>${types[i].name}</td>
                    <td>
        ...
        typeDropdown.innerHTML=dropDownOptions;
...
function displayPurpose(){
    ...
    .then(data => {
        ...
        for (let i=0;i<purpose.length;i++){
            dropDownOptions+=`<option value="${purpose[i].id}">${purpose[i].name}</option>`;
            content+=`<tr>
                    <td>${purpose[i].id}</td>
        ...
        purposeDropdown.innerHTML=dropDownOptions;
...
function displayProperties(){
    ...
    .then(data => {
        ...
        for (let i=0;i<properties.length;i++){
            ...
            inputsContent+=`<p>
            <input type="text" class="form-control prop-input" placeholder="${properties[i].name} (${properties[i].units})" name="prop_${properties[i].id}"/>
            </p>`
        }
...
function displaySalts(){
    ...
    .then(data => {
        ...
        for (let i=0;i<salts.length;i++){
            ...
            for (j=0;j<salts[i].properties.length;j++){
                if(salts[i].properties[j].value !== null && salts[i].properties[j].value.trim() !== "") {
                    propertiesContent+=`
                    ${salts[i].properties[j].name}: ${salts[i].properties[j].value} ${salts[i].properties[j].units} </br>
                    `
                }
            }
            ...
            content+=`<tr>
                    ...
                    <td>${salts[i].typename}</td>
                    <td>${salts[i].purposename}</td>
                    <td>${salts[i].marking}</td>
 ...
 typesForm.addEventListener("submit", function(event) {
    ...
    saltsForm.addEventListener("submit", function(event) {
        ...
        const dataToSend = {
            ...
            type_id: document.querySelector('#saltForm select[name="type_id"]').value,
            purpose_id: document.querySelector('#saltForm select[name="purpose_id"]').value,
  ...
  if (event.target.classList.contains('delete-type-btn')) {
    ...
    .then(data => {
        let salt=data;
            ...
            document.querySelector('#saltForm select[name="type_id"]').value=salt.type_id,
            document.querySelector('#saltForm select[name="purpose_id"]').value=salt.purpose_id,
            ...
            for (let i=0;i<salt.properties.length;i++){
                document.querySelector('#saltForm input[name="prop_'+salt.properties[i].property_id+'"]').value=salt.properties[i].value;
            }
        });    
  }
