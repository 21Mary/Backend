...
class PurposeList extends BaseList{
    public function add($params){
        $elem=new Purpose($params['id'],$params['name']);
    ...
    public function getAllFromDatabaseById($id){
        global $conn;
        $stmt = $conn->prepare("SELECT * FROM purpose WHERE id=?");
    ...
    public function insertIntoDatabase($params){
        global $conn;
        $stmt = $conn->prepare("INSERT INTO purpose VALUES (DEFAULT, ?)");
    ...
    public function updateDatabaseById($params){
        global $conn;
        $stmt = $conn->prepare("UPDATE `purpose` SET `name`=? WHERE `id`=?;");
        $stmt->bind_param("ss", $params['name'],$params['id']);
        $stmt->execute();
    }
    public function deleteFromDatabaseById($id){
        global $conn;
        $stmt = $conn->prepare("DELETE FROM purpose WHERE id=?");
        $stmt->bind_param("s", $id);
        $stmt->execute();
    }
